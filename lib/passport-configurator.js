...
PassportConfigurator.prototype.configureProvider = function (name, options) {
  ...
  if (authType === 'local') {
    ...
    self.app.get(authPath, function (req, res, next) {
      var state = encodeURIComponent(req._parsedOriginalUrl.search || '');
      var opt = _.defaults({
        state: state,
        scope: scope,
        session: session
      }, options.authOptions);
      return passport.authenticate(name, opt)(req, res, next);
    });
  }
      ...
      passport.authenticate(name, _.defaults({session: session}, options.authOptions), function (err, user, info) {
        ...
        if (session) {
          req.logIn(user, function (err) {
            ...
            return res.redirect(successRedirect + decodeURIComponent(req.query.state || ''));
          });
        } else {
          if (info && info.accessToken) {
            if (!!options.json) {
